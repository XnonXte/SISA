...
export default function Konfirmasi({ go, userData }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#F7F9F7', position: 'relative' }}>
      ...
      <div style={{ flex: 1, padding: '0 20px', overflowY: 'auto' }}>
        ...
        <div style={{ background: '#FFF8E1', borderRadius: 20, padding: 20, marginTop: 24, boxShadow: '0 2px 12px rgba(245,166,35,0.1)' }}>
          ...
          {[
            ...
            ['Tujuan', <span><i className="bi bi-wallet2" style={{ marginRight: 6 }} />{REDEEM_DEST}</span>, '#1DB954'],
        ...
        <div style={{ padding: '0 8px', marginTop: 24, textAlign: 'center' }}>
      ...
      <div style={{ padding: '0 20px 16px', background: '#fff', borderTop: '1px solid #E0E0E0' }}>
        <button className="btn-primary" onClick={() => setShowSuccess(true)}>Tukar Sekarang</button>
        <button className="btn-secondary" style={{ marginTop: 12 }} onClick={() => go('dashboard')}>Batal</button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="success-modal" style={{ background: '#fff', borderRadius: 24, padding: '32px 24px', width: 320, textAlign: 'center' }}>
            <div style={{ marginBottom: 16 }}>
              <i className="bi bi-check-circle-fill" style={{ fontSize: 56, color: '#1DB954' }} />
            </div>
